        ...
        switch (module)
        {
            ...
            case kDFMini:
                ...
                if (!fDFMini.begin(stream))
                {
                    ...
                    // JRS - Why is this returning false?
                    //Think it might be this from DFRobotDFPlayerMini.cpp (perhaps the timeout):
                            // if (doReset) {
                            //   reset();
                            //   waitAvailable(2000);
                            //   delay(200);
    ...
    bool handleCommand(const char* cmd, bool skipStart = false)
    {
        ...
        switch(cmdch)
        {
            case 'R':   // R - random from 4 first banks
                Serial.print(F("\r\n MarcduinoSound char random "));

                startRandom(); // keep firing random sounds
                break;
            case 'O':   // O - sound off
                Serial.print(F("\r\n MarcduinoSound char off "));

                stopRandom();
                volumeOff();
                break;
            case 'L':   // L - Leia message (bank 7 sound 1)
                Serial.print(F("\r\n MarcduinoSound char leia "));
            ...
            case 'C':   // C - Cantina music (bank 9 sound 5)
                Serial.print(F("\r\n MarcduinoSound char cantina music "));
            ...
            case 'c':   // c - Beep cantina (bank 9 sound 1)
                Serial.print(F("\r\n MarcduinoSound char cantina beep "));
            ...
            case 'F':   // F - Faint/Short Circuit (bank 6 sound 3)
                Serial.print(F("\r\n MarcduinoSound char short circuit"));
            ...
            case 's':   // s - stop sounds
                Serial.print(F("\r\n MarcduinoSound char stop sounds "));

                stopRandom();
                stop();
                break;
            case '+':   // + - volume up
                Serial.print(F("\r\n MarcduinoSound char vol up "));

                volumeUp();
                break;
            case '-':   // - - volume down
                Serial.print(F("\r\n MarcduinoSound char vol down "));

                volumeDown();
                break;
            case 'm':   // m - volume mid
                Serial.print(F("\r\n MarcduinoSound char vol mid "));

                volumeMid();
                break;
            case 'f':   // f - volume max
                Serial.print(F("\r\n MarcduinoSound char vol max "));

                volumeMax();
                break;
            case 'p':   // p - volume min
                Serial.print(F("\r\n MarcduinoSound char vol min "));

                volumeMin();
                break;
            case 'W':   // W - Star Wars music (bank 9 sound 2)
                Serial.print(F("\r\n MarcduinoSound char SW music "));

                stopRandom();      // so long, just stop random
                playSound(8,2);
                break;
            case 'M':   // M - Imperial March (bank 9 sound 3)
                Serial.print(F("\r\n MarcduinoSound char SW march "));

                stopRandom();      // so long, just stop random
                playSound(8,3);
                break;
            default:
                Serial.print(F("\r\n MarcduinoSound char false "));
